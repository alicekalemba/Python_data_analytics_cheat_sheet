...
### dropna()
This is a method in pandas used to remove missing values (like NaNs or None) 
from a DataFrame or Series.

_Example_

`Suppose you have the following DataFrame containing some missing values:
data = {
    'A': [1, 2, np.nan, 4],
    'B': [5, np.nan, np.nan, 8],
    'C': [10, 11, 12, 13]
...
To remove rows with any missing values, use `dropna` like this:
...
However, if you want to remove columns with any missing values instead, specify the `axis` 
parameter: 
